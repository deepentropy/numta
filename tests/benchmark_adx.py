"""
Benchmark comparing talib-pure ADX performance with original TA-Lib
"""
...
def benchmark_adx():
    """Benchmark ADX function against TA-Lib"""
    try:
        import talib
    ...
    print("=" * 60)
    print("Average Directional Movement Index (ADX) Performance Benchmark")
    print("=" * 60)
    ...
    for n in sizes:
        ...
        # Warmup
        _ = ADX_pure(high, low, close)

        iterations = 1000 if n <= 1000 else 100
        start = time.perf_counter()
        for _ in range(iterations):
            result_pure = ADX_pure(high, low, close)
        ...
        if talib_available:
            # Warmup
            _ = talib.ADX(high, low, close)

            start = time.perf_counter()
            for _ in range(iterations):
                result_talib = talib.ADX(high, low, close)
            end = time.perf_counter()
